-- for k in pairs(io) do print(k) end
-- Все что есть в io:

-- open --> открывет файл по его имени с указанным режимом
-- popen --> позволяет запускать внешние команды операционной системы и взаимодействовать с ними через канал (pipe) (есть пример снизу)
-- tmpfile --> возвращает дескриптор временного файла. Этот файл открывается в режиме обновления и автоматически удаляется по завершении работы программы
-- input --> при вызове с именем файла он открывает указанный файл (в текстовом режиме) и устанавливает его дескриптор в качестве входного файла по умолчанию. Возвращает дескриптор stdin
-- output --> аналогично input только выходной файл по умолчанию
-- stdin --> возвращает соотв. дескриптор 
-- stdout --> возвращает соотв. дескриптор 
-- stderr --> возвращает соотв. дескриптор 
-- type --> проверяет, является ли объект допустимым дескриптором файла
-- close --> применяет все отложенные операции записи, закрывает файл
-- read --> читает из файла
-- write --> записывает файл
-- flush --> применяет к файлу все отложенные операции записи
...
-- случае ошибки open возвращает nil, а также сообщение об ошибке и ее код
...
-- пример работы с popen

local file = assert(io.popen("ls", "r"))
