...
-- - модификатор, обозначающий 0 или более коротких повторений (пример ниже)
...
--                       _       _        _                           _       
--                      (_)     | |      | |                         | |      
--   ___ _ __   ___  ___ _  __ _| |   ___| | ___ _ __ ___   ___ _ __ | |_ ___ 
--  / __| '_ \ / _ \/ __| |/ _` | |  / _ \ |/ _ \ '_ ` _ \ / _ \ '_ \| __/ __|
--  \__ \ |_) |  __/ (__| | (_| | | |  __/ |  __/ | | | | |  __/ | | | |_\__ \
--  |___/ .__/ \___|\___|_|\__,_|_|  \___|_|\___|_| |_| |_|\___|_| |_|\__|___/
--      | |                                                                   
--      |_|                                                                   

-- %b - элемент в образце, который соответствует сбалансированным строкам (у которых парные символы по краям)
-- %f - соответствует пустой строке, только если следующий символ входит во множество_символов, а предыдущий нет

-- Пример с %b
local text = "a (enclosed (in) parentheses) line"
print((string.gsub(text, "%b()", ""))) -- a  line

-- Пример с %f
text = "the anthem is the theme"
print((text:gsub("%f[%w]the%f[%W]", "one"))) -- one anthem is one theme
-- '%f[%w]' означает что предыдущий символ не входит в w, а следующий входит
-- '%f[%W]' означает что предыдущий символ входит в w, а следующий не входит
-- В данном случае:
-- '%f[%w]': предыдущий - пробел(не входит в w), а следующий - t (входит в w)
-- '%f[%W]': предыдущий - e(входит в w), а следующий - пробел (не входит в w)

-- Позиции перед первым и после последнего символов в
-- обрабатываемой строке трактуются как содержащие нуль-символ
-- (символ с ASCII кодом 0). В предыдущем примере первое "the"
-- начинается с границы между нуль-символом (не во множестве '[%w]') и 't'
-- (во множестве '[%w]').

--                   _                       
--                  | |                      
--    ___ __ _ _ __ | |_ _   _ _ __ ___  ___ 
--   / __/ _` | '_ \| __| | | | '__/ _ \/ __|
--  | (_| (_| | |_) | |_| |_| | | |  __/\__ \
--   \___\__,_| .__/ \__|\__,_|_|  \___||___/
--            | |                            
--            |_|                            

-- captures (захваты)

-- Механизм захвата (capture) позволяет образцу выдергивать из
-- обрабатываемой строки те части, которые удовлетворяют частям
-- образца, в целях дальнейшего использования. Вы можете указать захват
-- посредством записи захватываемых частей образца внутри круглых
-- скобок.

-- Когда в образце есть захваты, то функция string.match возвращает
-- каждое захваченное значение как отдельный результат; другими
-- словами, она разбивает строку на ее захваченные части.

-- нахождение ключей и значений в строке
local s = "name = Anna"
local re = "(%a+)%s*=%s*(%a+)"
local k, v = s:match(re)
print(
    string.format("key = %s, value = %s", k, v)
) -- key = name, value = Anna

-- нахождение даты в строке
local s = "Today is 17/7/1990"
local re = "(%d+)/(%d+)/(%d+)"
local d, m, y = s:match(re)

print(
    string.format("day = %s, month = %s, year = %s", d, m, y)
) -- day = 17, month = 7, year = 1990


--                  _                                     _       
--                 | |                                   | |      
--   _ __ ___ _ __ | | __ _  ___ ___ _ __ ___   ___ _ __ | |_ ___ 
--  | '__/ _ \ '_ \| |/ _` |/ __/ _ \ '_ ` _ \ / _ \ '_ \| __/ __|
--  | | |  __/ |_) | | (_| | (_|  __/ | | | | |  __/ | | | |_\__ \
--  |_|  \___| .__/|_|\__,_|\___\___|_| |_| |_|\___|_| |_|\__|___/
--           | |                                                  
--           |_|                                                  

-- replacement (замены)

-- Вместо строки в качестве третьего аргумента string.gsub мы можем
-- использовать функцию или таблицу. При вызове с функцией string.gsub
-- вызывает эту функцию каждый раз, когда находит соответствие;

local s = "hello Lua"
print((s:gsub("Lua", function (text)
    return string.upper(text)
end))) -- hello LUA

local text = "2 apples and 5 oranges"
print((text:gsub("%d+", function(num)
    return "["..num.."]"
end)))

-- Можно комбинировать с захватами, тогда функция будет принимать 
-- все что захватилось

local s = "hello Lua"
print((s:gsub("(L)(u)(a)", function(a, b, c)
    return a..string.upper(b)..c
end))) -- hello LUa

-- Смена местами всех соседних символов
local s = "hello Lua"
local re = "(.)(.)"
print((s:gsub(re, function(a, b)
    return b..a
end))) -- ehll ouLa

-- Практический пример: перевод всех DD.MM.YYYY в YYYY.MM.DD

local s = "Today is 27.07.2025, yesterday was 26.07.2025"
local re = "(%d+).(%d+).(%d+)"
print((s:gsub(re, function (d, m, y)
    return y.."."..m.."."..d
end))) -- Today is 2025.07.27, yesterday was 2025.07.26
...
-- нахождение всех дробных чисел в строке
print()
local text = "Temp: 36.6C, Price: 100.99$, 42 is magic, 0.5 apples"
local re = "%d+%.?%d+"
for n in text:gmatch(re) do
    print(n)
end

-- Подобно '*', модификатор '-' также соответствует нулю или
-- большему количеству символов заданного класса. Однако, вместо
-- соответствия самой длинной последовательности он соответствует
-- самой короткой.

-- Например задача - найти все комментарии си

local text = "int x; /* x */ int y; /* y */"
-- Пробую так
local re = "/%*.*%*/"
for comm in text:gmatch(re) do
    print(comm)
end
-- вывело 1 комментарий - /* x */ int y; /* y */
-- поскольку '.*'. длится столько,
-- сколько сможет, первые комбинация "/*" в вашей программе закроется
-- лишь самой последней "*/"
...
for comm in text:gmatch(re) do
    print(comm)
end -- вывело 2 комментария
-- /* x */
-- /* y */
-- потому что образец '.-' наоборот захватит наименьшее количество символов,
-- необходимое для нахождения первого сочетания '*/'

-- теперь можно, например, корректно удалить все комментарии
print((text:gsub(re, ""))) -- int x;  int y;

-- Если образец начинается с символа '^', то он будет сопоставляться
-- только с началом обрабатываемой строки. И точно так же, если образец
-- заканчивается символом '$', то он будет сопоставляться только с концом
-- обрабатываемой строки

-- Интересно что
-- Символы '^' и '$' являются магическими лишь тогда, когда применяются
-- в начале или конце образца. Иначе они служат обычными символами,
-- которые соответствуют самим себе.